...
//        [SerializeField] public string m_MapServerAddress;
...
//        private void Start()
//        {
//            m_Controller.onExitStreetLevel += OnStartFlying;
//            m_Controller.onEnterStreetLevel += OnStopFlying;
//        }

//        private void OnStartFlying()
//        {
//            m_Controller.onChangeZoom += OnMapChangeZoom;
//            OnMapChangeZoom();
...
//            Vector2 currentCoordinates = m_Map.GetCoordinates();

//            double distanceFromLastRequest = Utilities.DistanceBetweenPointsD(currentCoordinates, m_LastMarkerPosition) * 1000;
...
//        {
//            if (string.IsNullOrEmpty(m_MapServerAddress))
//                return;

//            m_Range = distance;
//            distance = (int)(distance * 1.5f);

//            m_LastMarkerPosition = m_Map.GetCoordinates();
...
//            m_RequestCoroutine = StartCoroutine(Utilities.GetRequest(
//                 m_MapServerAddress + "?latitude=" + lat.ToString().Replace(',', '.') + "&longitude=" + lng.ToString().Replace(',', '.') + "&radius=" + distance,
//                 (result, response) =>
//                 {
//                     m_RequestCoroutine = null;
//                     if (result == 200)
...
//        {
//            m_MarkerScale = LeanTween.easeOutCubic(m_MinScale, m_MaxScale, m_Map.normalizedZoom);
//            m_DetailedMarkers = m_Map.normalizedZoom > m_MarkerDetailedThreshold;
//            m_VisibleMarkers = m_Map.normalizedZoom > m_MarkerVisibleThreshoold;
...
//                {
//                    item = m_MarkerPool.Spawn(m_Map.itemContainer);
//                    item.transform.position = m_Map.GetWorldPosition(markers[i].longitude, markers[i].latitude);
