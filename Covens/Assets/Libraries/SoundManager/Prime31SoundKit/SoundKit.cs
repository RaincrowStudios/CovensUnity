...
public class SoundKit : MonoBehaviour
{
    ...
    /// <returns>The sound.</returns>
    /// <param name="audioClip">Audio clip.</param>
    public void playSoundDelay(AudioClip audioClip, float volumeScale, float pitch, float pan, float fDelay)
    {
        StartCoroutine(PlayScheduled(audioClip, volumeScale, pitch, pan, fDelay));
    }
    private IEnumerator PlayScheduled(AudioClip audioClip, float volumeScale, float pitch, float pan, float fDelay)
    {
        yield return new WaitForSeconds(fDelay);
        playSound(audioClip, volumeScale, pitch, pan);
    }
