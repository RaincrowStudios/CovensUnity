...
public class FTFManager : MonoBehaviour
{
	...
	IEnumerator MattContinueHelper(bool nextHasDialogue)
	{
		...
		if (curIndex == 1)
		{
			...
			LeanTween.scale(wildBarghestInstance, Vector3.one, .8f).setEase(easeType);
			StopRotation();
			zoomCamera(-200, 2.4f);
		...
		else if (curIndex == 5)
		{
			...
			//dialogueMidText.text = dialogueText.text;
			//TextMeshProUGUI icky = wildBarghestInstance.transform.GetChild(0).GetChild(0).GetChild(1).GetComponent<TextMeshProUGUI>();//GetComponentInChildren<TextMeshProUGUI>();
			//i.text = "Howdy";
			//icky.text = "Energy: <b><color=#4C80FD>" + "38" + "</color></b>";
		...
		else if (curIndex == 15)
		{
			...
			moveCamera ( new Vector3((brigPos.x - 36), (brigPos.y + 10), brigPos.z), 2f);
			rotateCamera (360, 2f);
			zoomCamera (-280f, 2f);
		...
		else if (curIndex == 51)
		{
			...
			LeanTween.alphaCanvas(storePrefab.GetComponent<CanvasGroup>(), 0f, 0.6f).setOnComplete(() => {
	...
	IEnumerator BarghestWildDefeat()
	{
		...
		LeanTween.scale(wildBarghestInstance, Vector3.zero, 0.3f).setEase(easeType).setOnComplete(() =>
			{
