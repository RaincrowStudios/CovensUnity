...
public class DownloadManager : MonoBehaviour
{
    ...
    private static IEnumerator StartDownloads(AssetResponse assets)
    {
        ...
        //check if the dictionary was already downloaded
        string localDictionaryPath = Path.Combine(Application.persistentDataPath, "dict.text");
        bool dictionaryDownloaded = false;

        if (PlayerPrefs.HasKey("DataDict"))
        {
            string currentDictionary = PlayerPrefs.GetString("DataDict");
            if (currentDictionary == assets.dictionary)
            {
                if (System.IO.File.Exists(localDictionaryPath))
                {
                    Debug.Log($"\"{assets.dictionary}\" already downloaded.");
                    ...
                    if (SaveDict(json))
                        ...
                        Debug.LogError("Failed to parse the existing dictionary. Redownloading.");
                }
            }
        }

        //download the dictionary
        if (dictionaryDownloaded == false)
        {
            Debug.Log($"Downloading \"{assets.dictionary}\"");
            OnDictionaryDownloadStart?.Invoke();

            bool fail = true;
            int retryCount = 0;

            UnityWebRequest www = UnityWebRequest.Get(DownloadManager.downloadUrl + assets.dictionary);

            //try to download the dictionary 3 times
            ...
            else
            {
                ...
                PlayerPrefs.SetString("DataDict", assets.dictionary);
                Debug.Log($"Downloaded new dictionary \"{assets.dictionary}\"");
