...
public class TeamManagerUI : MonoBehaviour
{
    ...
    {
        m_Instance = this;
        m_ViewMembersButton.transform.GetChild(1).GetChild(0).GetComponent<Text>().text = LocalizeLookUp.GetText("invite_member").Replace("{{member}}", "").Replace(":", "").Replace("ー", "");
    ...
    private void SetupHome()
    {
        ...
        if (m_CovenData.IsMember)
            m_Home.m_Motto.text = string.IsNullOrEmpty(m_CovenData.Motto) ? "\"" + LocalizeLookUp.GetText("coven_motto_here") + "\"" : "\"" + m_CovenData.Motto + "\"";
        else
            m_Home.m_Motto.text = string.IsNullOrEmpty(m_CovenData.Motto) ? m_CovenData.Name : "\"" + m_CovenData.Motto + "\"";
        ...
        if (m_CovenData.School < 0)
            m_Home.m_CovenSigil.overrideSprite = m_Home.shadowSchool;
    ...
    {
        TeamPlayerView.ViewCharacter(member.MemberData.Id, null, false, () => this.Hide());
    }
