...
{
    public class BOSSelectZoneView : BOSBase
    {
        ...
        void Start()
        {
            ...
            spawnRegion.text = LocalizeLookUp.GetText("ftf_spawn_region").Replace("{{region}}", LocalizeLookUp.GetZoneName(BOSSpiritView.currentZone));

            if (BOSSpiritView.discoveredSpirits == 0)
            {
        ...
        {
            SetButtons(true);
            for (int i = 0; i < BOSSpiritView.undiscoveredSpirits; i++)
            {
                var g = Utilities.InstantiateObject(undiscoveredCard, container);
            }
            spiritCountInfo.text = BOSSpiritView.undiscoveredSpirits.ToString() + " " + LocalizeLookUp.GetText("spirit_undiscovered");// " Undiscovered Spirits";
        }
        ...
        void showDiscoveredSpirits()
        {
            ...
            {
                SpiritData spirit = DownloadedAssets.GetSpirit(item.spirit);
                if (spirit.zones != null && spirit.zones.Contains(BOSSpiritView.currentZone))
                {
            ...
            spiritCountInfo.text = BOSSpiritView.discoveredSpirits.ToString() + " " + LocalizeLookUp.GetText("spirit_discovered");// Discovered Spirits";
